...
<<<<<<< HEAD
$options['heightmargin']=300; //حجم الصورة
$options['betweenmargin']=10; //حجم التباعد
$options['titletablemargin']=50; //بعد العنوان السفلي
$options['cellwidthmargin']=50; // "عرض الأعمدة "عدا العامود
$options['cellheightmargin']=25; //عرض الخلية
$options['subjectwidthmargin']=50; 
$options['linethickness']=4; //عرض  خط الرسم
...
for($i=1,$j= $begin;$j<=$end;$j->modify("+1 day")){
    ...
    writetext($font,$image,$curx + $options['cellwidthmargin'] + ($col[$cntcol] - $w) / 2, $cury + $options['cellheightmargin'] + ($row[$i] - $h) / 2,$dayweek[$j->format("w")],$options['textsize'],$options['textcolor']);
