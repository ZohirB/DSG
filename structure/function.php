...
<<<<<<< HEAD
    function check_mul_label($in_id,$to){
        if ($to%2 != 0){
            ...
            echo "<input id='".$in_id."' type='checkbox' onclick='toggle(this,".$to.");checkboxes_inc();'>";
=======
    function check_mul_label($in_id,$year,$sem){
        if ($sem == 1){
            echo "<input id='".$in_id."' type='checkbox' onclick='toggle(this,".$year.",".$sem.");checkboxes_inc();' value='555'>";
            echo "<label for='".$in_id."'>تحديد مواد الفصل الأول </label>";
        }
        else{
            echo "<input id='".$in_id."' type='checkbox' onclick='toggle(this,".$year.",".$sem.");checkboxes_inc();' value='555'>";
    ...
    function print_labels($s_year,$to,$in_id,$co_id){
        ...
        foreach ($GLOBALS['data'] as $key => $val) {
            ...
            if ($co == $co_id && $val['year'] == $s_year){
                ...
                checkbox_subject_label($key,$val);
=======
    function checkbox_subject_label($key,$val,$year,$sem){
        echo "<input id='" . $key . "' type='checkbox' value='" . $key . "' onclick='checkboxes_inc();' name='subject[]' d_year='".$year."'  d_sem='".$sem."'>";
        echo "<label for='" . $key . "'>" . $val['name2'] . "</label>";
    }

    // function print_labels (yearOfSubject,labelId)
    function print_labels($s_year,$in_id){
        check_mul_label($in_id,$s_year,1);    
        foreach ($GLOBALS['data'] as $key => $val) {
            if ($val['year'] == $s_year && $val['id_sem'] == 1) {
                checkbox_subject_label($key,$val,$s_year,1);
            }
        ...
        $in_id++;
        check_mul_label($in_id,$s_year,2);    
        foreach ($GLOBALS['data'] as $key => $val) {
            if ($val['year'] == $s_year && $val['id_sem'] == 2) {
                checkbox_subject_label($key,$val,$s_year,2);
>>>>>>> ea3bdd3567edd67c270e0ba83e2a45b9027b6b27
            }
            else if ($val['year'] > $s_year){
                break;
